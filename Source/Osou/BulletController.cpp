...
void ABulletController::BeginPlay()
{
	...
	if (levelIndex == 0) {
		...
		player->rhythmDelayConstants = { 0, 0, 0};
		player->SetTransitions({0, 15.5+2, 105+2 }); //105
		player->setSpeedMultis({1, 1, 1, 1 });
		//player->AddRythm({ 0.28 }, 0);
	...
	if (spawner->spawnTable[index].time < clockTime) {
		...
		if (levelIndex == 0) {
			if (tutorialTime < 18) {
				...
				if (player->Health < 0.1 || player->hScore > 1) {
					if (player->hScore > 1) {
						DisplayMessage(FString("Dont get hit too many times, try moving in a triangle"));
					}
					else {
...
void ABulletController::ResetBullets()
{
	...
	if (levelIndex == 2) {
		DisplayMessage("This level is hard try some Touhou levels if you are having trouble");
	}
